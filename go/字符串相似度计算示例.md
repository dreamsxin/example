	...
	for i := 1; i <= n; i++ {
		for j := 1; j <= m; j++ {
			if r1[i-1] == r2[j-1] {
				...
				transposeCost := math.MaxFloat64 // 初始化为极大值
...
func (sc *SpellChecker) Correct(word string) []SpellSuggestion {
	...
	for _, dictWord := range sc.Dictionary {
		...
		if distance <= float64(sc.MaxDistance) {
			// 计算最大长度
			maxLen := len(word)
			if len(dictWord) > maxLen {
				maxLen = len(dictWord)
			...
			// 计算归一化相似度
			confidence := 1.0 - distance/float64(maxLen)
...
func (sw *SmithWaterman) Align(seq1, seq2 string) (float64, string, string) {
	...
	for i := 1; i <= m; i++ {
		for j := 1; j <= n; j++ {
			...
			// 处理gap - 需要区分gap open和gap extend
			// 对于垂直gap（seq1缺失）
			var up float64
			if traceback[i-1][j] == 2 { // 如果上一个就是gap，则是gap extend
				up = score[i-1][j] + sw.GapExtendPenalty
			} else { // 否则是gap open
				up = score[i-1][j] + sw.GapOpenPenalty + sw.GapExtendPenalty
			}
			
			// 对于水平gap（seq2缺失）
			var left float64
			if traceback[i][j-1] == 3 { // 如果左边就是gap，则是gap extend
				left = score[i][j-1] + sw.GapExtendPenalty
			} else { // 否则是gap open
				left = score[i][j-1] + sw.GapOpenPenalty + sw.GapExtendPenalty
			}
			
			// 取最大值，不小于0
			maxVal := math.Max(0, math.Max(diagonal, math.Max(up, left)))
			score[i][j] = maxVal
...
func main() {
	...
	for i, match := range matches {
		...
		fmt.Printf("  对齐长度: %d\n", match["alignment_length"].(int))
		
		// 显示对齐结果（限制长度）
		aligned1 := match["aligned1"].(string)
		aligned2 := match["aligned2"].(string)
		if len(aligned1) > 30 {
			fmt.Printf("  对齐1（前30个字符）: %s...\n", aligned1[:30])
			fmt.Printf("  对齐2（前30个字符）: %s...\n", aligned2[:30])
		} else {
			fmt.Printf("  对齐1: %s\n", aligned1)
			fmt.Printf("  对齐2: %s\n", aligned2)
		}
	...
	text4 := "A quick brown fox jumps over a lazy dog."
	matches2 := detector.Detect(text3, text4)
	fmt.Printf("发现 %d 个相似片段:\n", len(matches2))
}
```
...
func AnalyzeHamming(s1, s2 string) map[string]interface{} {
	...
	// 修复：Go没有三元运算符，需要用if-else
	if distance > 0 {
		result["bits_per_error"] = float64(length) / float64(distance)
	} else {
		result["bits_per_error"] = 0.0
	}
...
type RecommendationSystem struct {
	...
	UserPrefs       map[string][]float64          // 用户偏好向量（基于物品特征的向量）
	UserItemRatings map[string]map[string]float64 // 用户对物品的评分
}
...
func NewRecommendationSystem() *RecommendationSystem {
	return &RecommendationSystem{
		...
		UserItemRatings: make(map[string]map[string]float64),
	}
}
...
// 添加用户对物品的评分
func (rs *RecommendationSystem) AddUserItemRating(userID, itemID string, rating float64) {
	if rs.UserItemRatings[userID] == nil {
		rs.UserItemRatings[userID] = make(map[string]float64)
	}
	rs.UserItemRatings[userID][itemID] = rating
}
...
func (rs *RecommendationSystem) CollaborativeFiltering(userID string, topN int) []map[string]interface{} {
	targetUserRatings, exists := rs.UserItemRatings[userID]
	if !exists {
		return nil
	}
	...
	for otherUserID, otherUserRatings := range rs.UserItemRatings {
		if otherUserID == userID {
			continue
		}

		// 为两个用户创建共同物品的向量
		var targetVec, otherVec []float64
		for itemID, targetRating := range targetUserRatings {
			if otherRating, exists := otherUserRatings[itemID]; exists {
				targetVec = append(targetVec, targetRating)
				otherVec = append(otherVec, otherRating)
			}
		}
		...
		similarity := CosineSimilarity(targetVec, otherVec)
		if similarity > 0 {
	...
	for i := 0; i < k; i++ {
		...
		// 获取相似用户评价过的物品
		similarUserRatings := rs.UserItemRatings[similarUserID]

		// 找到目标用户没有评价过的物品
		for itemID, rating := range similarUserRatings {
			if _, exists := targetUserRatings[itemID]; !exists {
...
func main() {
	...
	// 添加用户对物品的评分（协同过滤需要）
	rs.AddUserItemRating("user1", "item1", 5.0)
	rs.AddUserItemRating("user1", "item2", 1.0)
	rs.AddUserItemRating("user1", "item3", 3.0)

	rs.AddUserItemRating("user2", "item1", 4.0)
	rs.AddUserItemRating("user2", "item3", 5.0)
	rs.AddUserItemRating("user2", "item4", 2.0)

	rs.AddUserItemRating("user3", "item2", 5.0)
	rs.AddUserItemRating("user3", "item4", 4.0)
	rs.AddUserItemRating("user3", "item5", 3.0)
