...
```sql
ALTER TABLE default.search_log
MODIFY TTL updated_at + INTERVAL 30 DAY;

-- 删除过期数据到其他磁盘卷
TTL created_at + INTERVAL 30 DAY TO VOLUME 'cold_volume'

-- 删除到指定磁盘后完全删除
TTL created_at + INTERVAL 30 DAY TO DISK 'cold_disk',
    created_at + INTERVAL 60 DAY DELETE

-- 按不同条件设置不同TTL
TTL 
    created_at + INTERVAL 30 DAY WHERE status = 1,
    created_at + INTERVAL 7 DAY WHERE status = 0
```
